...
function UserTransactionHistory() {
    ...
    const updateAfterCancel = (billID) => {
        ...
        axios
            ...
            .then((res) => {
                let notification={type:"NOTIFICATION",content:account.username + " canceled the booking " + res.data.house.name,url: `/myaccount/see_reviews/${res.data.vendor.id}`, account: {id:res.data.vendor.id }}
